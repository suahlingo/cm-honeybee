...
// Register a new SavedLegacyInfo record
func SavedLegacyInfoRegister(savedLegacyInfo *model.SavedLegacyInfo) (*model.SavedLegacyInfo, error) {
	result := db.DB.Create(savedLegacyInfo)
	err := result.Error
	if err != nil {
		return nil, err
	}

	return savedLegacyInfo, nil
}

// Get SavedLegacyInfo by connection ID
func SavedLegacyInfoGet(connectionID string) (*model.SavedLegacyInfo, error) {
	savedLegacyInfo := &model.SavedLegacyInfo{}

	result := db.DB.Where("connection_id = ?", connectionID).First(savedLegacyInfo)
	err := result.Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("SavedLegacyInfo not found with the provided connection_id")
		}
		return nil, err
	}
...
// Update SavedLegacyInfo by connection ID
func SavedLegacyInfoUpdate(savedLegacyInfo *model.SavedLegacyInfo) error {
	result := db.DB.Model(&model.SavedLegacyInfo{}).
		Where("connection_id = ?", savedLegacyInfo.ConnectionID).
		Updates(savedLegacyInfo)
	err := result.Error
	if err != nil {
		return err
	}
...
// Delete SavedLegacyInfo record
func SavedLegacyInfoDelete(savedLegacyInfo *model.SavedLegacyInfo) error {
	result := db.DB.Delete(savedLegacyInfo)
	err := result.Error
	if err != nil {
		return err
	}
